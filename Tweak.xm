...
// 自定义弹窗视图控制器
@interface GameChoiceViewController : UIViewController
@property (nonatomic, assign) unsigned int gameType;
@property (nonatomic, copy) void (^selectionHandler)(unsigned int gameContent);
@end
...
- (void)viewDidLoad {
	...
	// 顶部拖拽指示器
	_dragIndicator = [[UIView alloc] init];
	_dragIndicator.backgroundColor = [UIColor systemGray3Color];
	_dragIndicator.layer.cornerRadius = 2;
	_dragIndicator.translatesAutoresizingMaskIntoConstraints = NO;
	[_contentView addSubview:_dragIndicator];
	
	// 标题
	UILabel *titleLabel = [[UILabel alloc] init];
	titleLabel.textAlignment = NSTextAlignmentCenter;
	titleLabel.font = [UIFont systemFontOfSize:17 weight:UIFontWeightSemibold];
	titleLabel.text = (self.gameType == 1) ? @"请选择石头/剪刀/布" : @"请选择点数";
	...
	// 添加按钮
	NSArray *options = (self.gameType == 1) 
		? @[@"剪刀", @"石头", @"布"] 
		: @[@"1", @"2", @"3", @"4", @"5", @"6"];
	...
	for (NSString *option in options) {
		...
		[button setTag:[options indexOfObject:option]];
	...
	[NSLayoutConstraint activateConstraints:@[
		...
		[_contentView.widthAnchor constraintEqualToAnchor:self.view.widthAnchor multiplier:0.8],
		[_contentView.heightAnchor constraintLessThanOrEqualToAnchor:self.view.heightAnchor multiplier:0.7],
		
		// 拖拽指示器
		[_dragIndicator.topAnchor constraintEqualToAnchor:_contentView.topAnchor constant:12],
		[_dragIndicator.centerXAnchor constraintEqualToAnchor:_contentView.centerXAnchor],
		[_dragIndicator.widthAnchor constraintEqualToConstant:36],
		[_dragIndicator.heightAnchor constraintEqualToConstant:5],
		
		// 标题
		[titleLabel.topAnchor constraintEqualToAnchor:_dragIndicator.bottomAnchor constant:16],
		...
		// 按钮栈
		[_buttonStack.topAnchor constraintEqualToAnchor:titleLabel.bottomAnchor constant:24],
		[_buttonStack.leadingAnchor constraintEqualToAnchor:_contentView.leadingAnchor constant:20],
		[_buttonStack.trailingAnchor constraintEqualToAnchor:_contentView.trailingAnchor constant:-20],
		[_buttonStack.bottomAnchor constraintEqualToAnchor:_contentView.bottomAnchor constant:-24]
	]];
...
- (void)buttonTapped:(UIButton *)sender {
	unsigned int gameContent = (self.gameType == 1) 
		? (unsigned int)sender.tag + 1 
		: (unsigned int)sender.tag + 4;
...
- (void)handlePan:(UIPanGestureRecognizer *)gesture {
	...
	if (gesture.state == UIGestureRecognizerStateBegan) {
		...
		gesture.view.center = CGPointMake(initialCenter.x, initialCenter.y + translation.y);
	} 
	else if (gesture.state == UIGestureRecognizerStateEnded) {
		CGPoint velocity = [gesture velocityInView:self.view];
		if (velocity.y > 300 || gesture.view.center.y > self.view.bounds.size.height * 0.6) {
			[self dismiss];
		} else {
...
- (void)AddEmoticonMsg:(NSString *)msg MsgWrap:(CMessageWrap *)msgWrap {
	if (isGameCheatEnabled() && [msgWrap m_uiMessageType] == 47 && ([msgWrap m_uiGameType] == 2 || [msgWrap m_uiGameType] == 1)) {
		GameChoiceViewController *vc = [[GameChoiceViewController alloc] init];
		vc.gameType = [msgWrap m_uiGameType];
		...
		// 添加为子视图控制器（非模态）
		[topController addChildViewController:vc];
		vc.view.frame = topController.view.bounds;
		[topController.view addSubview:vc.view];
		[vc didMoveToParentViewController:topController];
		
		// 使用弱引用避免循环引用
		__weak GameChoiceViewController *weakVC = vc;
		vc.selectionHandler = ^(unsigned int gameContent) {
